...
import client.level.pathfinding.OldPathfindingTile.CurrentList;
import data.TileData;
import util.Util;

public class OldPathFinder implements TileData {
	private List<OldPathfindingTile> openList;//, closedList;
	private OldPathfindingTile[][] tiles;

	public OldPathFinder() {
		tiles = new OldPathfindingTile[Level.getHeight()][Level.getWidth()];
		for (int r = 0;r<Level.getHeight();r++) {
			for (int c = 0;c<Level.getWidth();c++) {
				tiles[r][c] = new OldPathfindingTile(c, r);
			}
		}
	}
	...
	public List<Point> getPath(int x1, int y1, int x2, int y2) {
		...
		List<OldPathfindingTile> path = findPath(x1, y1, x2, y2);
		for (OldPathfindingTile tileNode:path) {
			pathPoints.add(new Point(tileNode.getC(), tileNode.getR()));
		}
		return pathPoints;
	}

	public List<OldPathfindingTile> findPath(int iC, int iR, int fC, int fR) {
		for (int r = 0;r<Level.getHeight();r++) {
			for (int c = 0;c<Level.getWidth();c++) {
		...
		OldPathfindingTile currentPathfindingTile;
		while (openList.size()>0) {
			...
			List<OldPathfindingTile> adjacentTiles = getAdjacents(currentPathfindingTile);
			for (int i = 0;i<adjacentTiles.size();i++) {
				OldPathfindingTile currentAdjacent = adjacentTiles.get(i);
	...
	private List<OldPathfindingTile> calcPath(OldPathfindingTile start, OldPathfindingTile goal) {//starts at goal, doesnt include start
		LinkedList<OldPathfindingTile> path = new LinkedList<>();
		OldPathfindingTile curr = goal;
		while (true) {
			path.addFirst(curr);
			if (curr==start) break;
	...
	private OldPathfindingTile getLowestCombinedInOpen() {
		OldPathfindingTile min = openList.get(0);
	...
	private List<OldPathfindingTile> getAdjacents(OldPathfindingTile tile) {
		int x = tile.getC(), y = tile.getR();
		List<OldPathfindingTile> adjacent = new LinkedList<>();
		OldPathfindingTile temp;
		if (y>0) {
			temp = tiles[y-1][x];
